...
**Codex release review note (2026-05-13):** Added direct-script `src` path bootstrapping for the shadow monitor, replay evidence generator, and threshold calibration scripts so child sessions can run `python3 scripts/...` without manually setting `PYTHONPATH`. Release review also fixed the promotion-review API/workbench so an older review-request artifact is hydrated with the current replay/backtest evidence index at read time; this prevents the Dashboard from showing `missing=5` after M91 evidence artifacts exist. Release verification passed with `node --check web/dashboard/app.js`, Python compile checks, focused M91-M94 pytest (`168 passed, 1 skipped, 1 subtests passed`), post-fix focused pytest (`107 passed, 1 skipped, 1 subtests passed`), full `PYTHONPATH=src:. pytest -q` (`463 passed, 3 skipped, 10 subtests passed`), `git diff --check`, direct script dry-runs, and secret/path scans. Release tag: `pgc-v0.1.0-20260513-m91-m94-r2`.
