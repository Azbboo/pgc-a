...
| Latest deployed commit | See release tag `pgc-v0.1.0-20260514-m104-m107-plan-r1` |
| Latest release tag | `pgc-v0.1.0-20260514-m104-m107-plan-r1` |
...
| Latest full verification | M100-M103 verification passed on 2026-05-14: `node --check web/dashboard/app.js`; `bash -n scripts/run_daily_pipeline.sh`; `python3 -m compileall -q src scripts`; focused checks for Dashboard, market review/report/API, daily pipeline, runbook, and shadow governance; full `PYTHONPATH=src:. pytest -q` (`482 passed, 3 skipped, 10 subtests passed`); `git diff --check`. Runtime deploy `pgc-v0.1.0-20260514-m100-m103-cn-ops-quality-r1` ran remote release tests (`485 tests`, `skipped=3`) and remote `ops health --require-current-migrations` passed. Follow-up docs/ledger plan release `pgc-v0.1.0-20260514-m104-m107-plan-r1` keeps the same runtime code plus the M104-M107 task plan. M100-M103 remain advisory/read-only where applicable; no strategy/trade/position/paper-live/timer mutation was introduced outside existing guarded pipeline/ledger paths. |
...
| M100-M103 Chinese ops quality wave | `docs/plans/2026-05-13-m100-m103-cn-ops-quality-plan.md` |
...
| M95 | Rejected evidence source closure | Done, Deployed | `3bd691e`, release `pgc-v0.1.0-20260513-m95-m99-cn-r2` | Refreshed preconfirm and pullback dip-buy source evidence through 20260513 using local SQLite market bars overlay, added dip-buy T1 metrics, regenerated 20260513 replay/backtest evidence, and closed stale/metric-gap blockers without loosening validation. Verification: evidence preview `accepted=5 rejected=0`; focused pytest `16 passed`; release tests `475 tests`, `skipped=3`; remote health passed; safety scan confirmed artifact-only/no promote/trade/paper-live/timer mutation. |
| M96 | Shadow walk-forward outcome accumulator | Done, Deployed | `3bd691e`, release `pgc-v0.1.0-20260513-m95-m99-cn-r2` | Added date-scoped `shadow_walk_forward_outcomes_v1` artifact accumulation from shadow monitor signals and `market_bars`, including T+1/T+5 availability, partial-horizon/missing-bar/no-future diagnostics, monitor output links, daily pipeline summary fields, and daily report JSON/Markdown sections without strategy/trade/position/paper-live/timer writes. Verification: monitor generation command, focused M96 pytest `33 passed`, release tests `475 tests`, `skipped=3`, and remote health passed. |
| M97 | Shadow experiment registry | Done, Deployed | `3bd691e`, release `pgc-v0.1.0-20260513-m95-m99-cn-r2` | Added artifact-only `shadow_strategy_experiment_registry_v1` JSON/Markdown registry from M94 calibration recommendations, with required evidence, stop rules, frozen-CPB comparison, rollback rules, manual approval boundaries, and reviewer/script coverage. Verification: script generation, focused M97 pytest `45 passed`, Python compile, `git diff --check`, safety field check, scoped path/secret scans, release tests, and remote health passed. |
| M98 | Chinese shadow decision memo workbench | Done, Deployed | `3bd691e`, release `pgc-v0.1.0-20260513-m95-m99-cn-r2` | Added read-only `shadow_decision_memo_v1` API/Dashboard/report view linking review request, replay evidence, walk-forward outcomes, calibration, and experiment registry with Chinese sections for 候选概览、证据状态、阻断原因、下一步实验、人工决策、风险/回滚边界. No approve/promote/trade/plan/timer controls. Verification: focused M98 pytest `67 passed, 1 skipped`; full pytest `471 passed, 3 skipped, 10 subtests passed`; release tests and remote health passed. |
| M99 | Dashboard Chinese-first UX pass | Done, Deployed | `3bd691e`, release `pgc-v0.1.0-20260513-m95-m99-cn-r2` | Localized user-visible Dashboard copy across execution, review/history, paper acceptance, market diagnostics, strategy hypothesis, Shadow Lab, detail drawers, and TradingAgents surfaces. Replaced visible `paper/live`, `advisory`, `open-execution`, `ranked signals`, `source_refs`, `external_data_coverage`, `Rank/Score/Coverage/Blockers`, and raw shadow candidate keys with Chinese-first labels while keeping API paths and internal contract keys intact. Verification: `node --check web/dashboard/app.js`; Dashboard static checks `30 passed, 1 skipped`; focused M95-M99 pytest `103 passed, 1 skipped`; full pytest `472 passed, 3 skipped, 10 subtests passed`; release tests and remote health passed. |
| M100 | Dashboard Chinese IA and detail surfaces | Done, Deployed | `605ee4a`, release `pgc-v0.1.0-20260514-m100-m103-cn-ops-quality-r1` | Added M100 static guards for remaining visible-English regressions and dense detail surfaces; expanded display translations for ops/shadow/market/action-log fields; replaced raw JSON-like detail rows with grouped Chinese detail cards (结论、证据、阻断原因、下一步、来源); localized remaining visible status strings; grouped Shadow Lab decision/review and history/observation panels without adding write controls. Verification: `node --check web/dashboard/app.js`; Dashboard static checks `32 passed, 1 skipped`; full pytest `482 passed, 3 skipped, 10 subtests passed`; release tests `485 tests`, `skipped=3`; remote health passed. |
| M101 | Full-market review narrative depth | Done, Deployed | `605ee4a`, release `pgc-v0.1.0-20260514-m100-m103-cn-ops-quality-r1` | Added stored-evidence-only Chinese narrative payload for market regime, sector ranking reasons, representative stock reasons, freshness, evidence gaps, continuity judgement, and next-day plan relationship; surfaced concise Markdown sections in daily reports while preserving existing JSON/report fields. Verification: market review + daily report + API focused tests `47 passed, 3 subtests passed`; Python compile; full pytest and remote health passed. |
| M102 | Daily data and stock-pool intake operating loop | Done, Deployed | `605ee4a`, release `pgc-v0.1.0-20260514-m100-m103-cn-ops-quality-r1` | Added daily pipeline operating-state output (`data_refresh_needed`, `evidence_pack_needed`, `pool_intake_pending`, `dry_run_ready`, `apply_blocked`, `apply_complete`, `duplicate_apply_blocked`), linked stock-pool intake audit counts into CLI/script output, added service duplicate-apply blocking with explicit `--allow-rerun`, ran script preflight from `run_daily_pipeline.sh`, and documented child-session recovery. Codex review fixed a dry-run regression so completed apply writes no longer block later dry-run review. Verification: `bash -n scripts/run_daily_pipeline.sh`; script regression tests `7 passed`; focused M102 pytest `42 passed`; full pytest and remote release tests passed. |
| M103 | Shadow strategy decision governance | Done, Deployed | `605ee4a`, release `pgc-v0.1.0-20260514-m100-m103-cn-ops-quality-r1` | Added normalized `shadow_strategy_decision_queue_v1` across shadow decision memo, monitor output, scorecard artifacts, and experiment registry; each candidate now carries readiness, evidence status, walk-forward sufficiency, experiment status, required human decision, stop rules, next review date, and promotion boundary. Registry now references latest scorecard/walk-forward outcomes and current blockers; monitor links current registry stop rules. Regenerated 20260513 shadow artifacts. Verification: focused M103 pytest `38 passed`; direct monitor/registry script smokes; full pytest and remote health passed. Advisory-only: no strategy/trade/position/paper-live/timer writes. |
| M104 | Dashboard production visual QA | Done | `reports/dashboard_visual_qa_20260514.md`; `reports/m104_visual_qa_20260514/` | Captured desktop/mobile Dashboard QA for 开盘执行、每日复盘、全市场复盘、证据/运维、影子策略; removed remote Google Fonts dependency; fixed wrapping/min-width protections for nav labels, headings, toolbars, chips, notes, and mobile history pills; localized targeted dynamic shadow-review values. Diagnostics: `scrollWidth == clientWidth` on desktop/mobile and targeted English flags absent. Verification: Dashboard static checks `33 passed, 1 skipped`; `node --check web/dashboard/app.js`; full pytest `487 passed, 3 skipped, 10 subtests passed`; `git diff --check`. |
| M105 | Latest daily data and stock-pool production runbook | Done | `reports/daily_pipeline_production_acceptance_20260514.md`; `reports/daily_review_20260514.md` | Added M105 production runbook sequence for backup, latest-closed resolution, market refresh, pool intake dry-run/apply, preflight, dry-run/apply, post-apply dry-run review, reports, and health. `ops daily-preflight` now surfaces pool-intake audit counts, post-apply dry-run emits `post_apply_review=dry_run_allowed`, and service next-command points to report/backup/health review instead of silent re-apply. Latest-day evidence: 2026-05-14 initially blocked before market refresh, then guarded apply passed with `market_data count=263`, `pool_intake_added_count=6`, `daily_operating_state=apply_complete`, backup `data/backups/pgc_trading_20260514_185518_998714_before_daily_pipeline_20260514.db`, and refreshed 20260514 Markdown/JSON reports. Verification: M105 focused pytest `87 passed, 1 subtests passed`; full pytest `487 passed, 3 skipped, 10 subtests passed`; compile; `bash -n`; `git diff --check`. |
| M106 | Reviewed evidence-pack automation for market/Agent gaps | Done | `src/pgc_trading/services/evidence_provider_pack_service.py`; `tests/test_evidence_provider_pack_service.py` | Added provider-pack `qa_summary` into result/manifest output, covering closed gaps, remaining gaps, reviewed-unavailable states, missing provider sections, stale/duplicate evidence, invalid date/provider/source-hash records, and provider files needing review. CLI now emits compact `qa_summary_json` for `ops evidence-pack`. Verification: focused M106 pytest `22 passed`; evidence-pack CLI route tests passed via focused CLI set; full pytest `487 passed, 3 skipped, 10 subtests passed`; Python compile; `git diff --check`. Reviewed files only; no live fetches or trading/strategy state writes. |
| M107 | Shadow-to-paper manual promotion preflight | Done | `reports/shadow_paper_preflight_20260513.{json,md}`; `reports/shadow_paper_preflight_20260514.{json,md}` | Added manual-only `shadow_paper_preflight_v1` package with readiness score, evidence sufficiency, walk-forward sufficiency, stop-rule status, risk/rollback notes, required human approvals, and hard `paper_candidate_allowed=false` for every candidate. Regenerated 20260513/20260514 shadow artifacts; latest available market bars remain `20260513`, while `20260514` is a next-trade-date package without local close data. Verification: M107 focused pytest `38 passed`; full pytest `487 passed, 3 skipped, 10 subtests passed`; Python compile; `git diff --check`. No strategy-version, trade plan, trade, position, paper/live, broker, or timer mutation. |
...
| --- | --- | --- | --- | --- |
| A | M104 Dashboard production visual QA | Done | M100 Chinese IA, deployed Dashboard on port `8020` | Visual QA completed with desktop/mobile screenshots, no global overflow diagnostics, remote font removal, wrapping protections, and targeted dynamic English cleanup. Verification: Dashboard static checks `33 passed, 1 skipped`; full pytest `487 passed, 3 skipped, 10 subtests passed`. |
| B | M105 Latest daily data and stock-pool production runbook | Done | M102 daily operating loop, M67 pool intake | Production runbook and child-session evidence path completed; `20260514` initial missing-data block was followed by guarded apply after market refresh and pool intake, with backup/report evidence recorded; post-apply dry-run review remains usable with duplicate apply guarded. |
| C | M106 Reviewed evidence-pack automation for market/Agent gaps | Done | M63/M68 evidence packs, M101 narrative gaps | Provider-pack manifest QA now tells operators which evidence gaps are closed, which remain explicit, and which provider files or missing sections need review; import/report paths remain reviewed-file-only with no live fetch. |
| D | M107 Shadow-to-paper manual promotion preflight | Done | M103 decision queue, M95-M99 shadow evidence | Manual-only `shadow_paper_preflight_v1` package generated from latest 20260513 shadow evidence; all candidates remain blocked with `paper_candidate_allowed=false` and protected mutation targets unchanged. |
