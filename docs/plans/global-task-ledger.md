...
| M100 | Dashboard Chinese IA and detail surfaces | Done | local workspace; `docs/plans/2026-05-13-m100-m103-cn-ops-quality-plan.md` | Added M100 static guards for remaining visible-English regressions and dense detail surfaces; expanded display translations for ops/shadow/market/action-log fields; replaced raw JSON-like detail rows with grouped Chinese detail cards (结论、证据、阻断原因、下一步、来源); localized remaining visible status strings; grouped Shadow Lab decision/review and history/observation panels without adding write controls. Verification: `node --check web/dashboard/app.js`; Dashboard static checks `32 passed, 1 skipped`; `git diff --check`. |
| M101 | Full-market review narrative depth | Done | `codex/m101-full-market-review-narrative`; `docs/plans/2026-05-13-m100-m103-cn-ops-quality-plan.md` | Added stored-evidence-only Chinese narrative payload for market regime, sector ranking reasons, representative stock reasons, freshness, evidence gaps, continuity judgement, and next-day plan relationship; surfaced concise Markdown sections in daily reports while preserving existing JSON/report fields. Verification: market review + daily report focused tests `22 passed, 3 subtests passed`; API read routes `25 passed`; Python compile; `git diff --check`. |
| M102 | Daily data and stock-pool intake operating loop | Done | `docs/plans/2026-05-13-m100-m103-cn-ops-quality-plan.md` | Added daily pipeline operating-state output (`data_refresh_needed`, `evidence_pack_needed`, `pool_intake_pending`, `dry_run_ready`, `apply_blocked`, `apply_complete`, `duplicate_apply_blocked`), linked stock-pool intake audit counts into CLI/script output, added service duplicate-apply blocking with explicit `--allow-rerun`, ran script preflight from `run_daily_pipeline.sh`, and documented child-session recovery. Codex review fixed a dry-run regression so completed apply writes no longer block later dry-run review. Verification: `bash -n scripts/run_daily_pipeline.sh`; script regression tests `7 passed`; focused M102 pytest `42 passed`; full pytest `482 passed, 3 skipped, 10 subtests passed`; `git diff --check`. |
| M103 | Shadow strategy decision governance | Done | `docs/plans/2026-05-13-m100-m103-cn-ops-quality-plan.md` | Added normalized `shadow_strategy_decision_queue_v1` across shadow decision memo, monitor output, scorecard artifacts, and experiment registry; each candidate now carries readiness, evidence status, walk-forward sufficiency, experiment status, required human decision, stop rules, next review date, and promotion boundary. Registry now references latest scorecard/walk-forward outcomes and current blockers; monitor links current registry stop rules. Regenerated 20260513 shadow artifacts. Verification: focused M103 pytest `38 passed`; direct monitor/registry script smokes; full pytest `482 passed, 3 skipped, 10 subtests passed`; `node --check web/dashboard/app.js`; `bash -n scripts/run_daily_pipeline.sh`; Python compile; `git diff --check`. Advisory-only: no strategy/trade/position/paper-live/timer writes. |
...
| --- | --- | --- | --- | --- |
| A | M100 Dashboard Chinese IA and detail surfaces | Done | M99 Chinese-first pass | Completed Chinese-first detail grouping, raw-key translation coverage, Shadow Lab layout grouping, and Dashboard write-gate preservation. Verification: `node --check web/dashboard/app.js`; Dashboard static checks `32 passed, 1 skipped`; `git diff --check`. |
| B | M101 Full-market review narrative depth | Done | M48/M76 market hierarchy, M77 evidence ledger | Completed Chinese regime/sector/stock/evidence/continuity/plan narrative using stored review data only; missing sector/news/sentiment/plan evidence remains explicit as 缺失/不可用/证据不足. Verification: market review + daily report focused tests `22 passed, 3 subtests passed`; API read routes `25 passed`; Python compile; `git diff --check`. |
| C | M102 Daily data and stock-pool intake operating loop | Done | M25/M42 daily pipeline, M67 stock-pool intake, M75 ops preflight | Completed one repeatable daily path for refresh, pool intake, preflight, dry-run, apply, report, audit, duplicate-write handling, and pool intake artifact visibility; Codex review fixed dry-run review after completed apply writes. Verification: `bash -n scripts/run_daily_pipeline.sh`; script tests `7 passed`; focused M102 pytest `42 passed`; full pytest `482 passed, 3 skipped, 10 subtests passed`; `git diff --check`. |
| D | M103 Shadow strategy decision governance | Done | M95-M99 shadow evidence and decision memo | Completed normalized decision queue, registry stop-rule linking, latest-outcome/blocker references, regenerated 20260513 artifacts, and advisory-only safety checks. Verification: focused M103 pytest `38 passed`; direct monitor/registry script smokes; full pytest `482 passed, 3 skipped, 10 subtests passed`; Python compile; `git diff --check`. |
