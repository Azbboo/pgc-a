...
非 dry-run 写入必须带 `operator`。`--apply` 模式会在写入前备份数据库，并在输出中记录 `backup_path`；没有备份不得继续写入。默认策略版本是当前 paper 部署版本；需要回放或验证其他版本时，才在底层 `ops daily-pipeline` 命令显式追加 `--strategy-version`。
