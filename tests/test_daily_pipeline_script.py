...
class DailyPipelineScriptTest(unittest.TestCase):
    def test_latest_closed_resolves_date_and_runs_dry_run(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertIn("ops_history_event=daily_pipeline_run", log_text)
            self.assertIn("ops_history_occurred_at=", log_text)
            self.assertIn(f"account={PAPER_ACCOUNT_KEY}", log_text)
            self.assertIn("mode=dry-run", log_text)
