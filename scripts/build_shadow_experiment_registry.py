#!/usr/bin/env python3
"""Build artifact-only shadow strategy experiment registry reports."""
...
def main() -> int:
    args = parse_args()
    result = generate_shadow_experiment_registry(
    ...
    print_shadow_experiment_registry_result(result, compact=args.compact)
    return 0 if result["ok"] else 1


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("--date", "--as-of-date", dest="date", help="optional registry date YYYYMMDD")
    parser.add_argument("--db-path", type=Path, default=DEFAULT_DB_PATH)
    parser.add_argument("--reports-dir", type=Path, default=DEFAULT_REPORTS_DIR)
    parser.add_argument("--calibration-artifact", type=Path, help="optional source calibration JSON path")
    parser.add_argument("--output", type=Path, help="optional JSON artifact output path")
    parser.add_argument("--operator", default="shadow-experiment-registry")
    parser.add_argument("--dry-run", action="store_true", help="build in memory without writing JSON/Markdown")
    parser.add_argument("--compact", action="store_true", help="omit the full JSON artifact payload")
    return parser.parse_args()


def generate_shadow_experiment_registry(
    *,
    db_path: Path,
    reports_dir: Path,
    ...
    result = service.build_shadow_experiment_registry(
        BuildShadowExperimentRegistryRequest(
            ...
            calibration_artifact_path=(
                str(calibration_artifact_path) if calibration_artifact_path is not None else None
            ),
            output_path=str(output_path) if output_path is not None else None,
        ),
        RequestContext(
            request_id="script-shadow-experiment-registry",
            dry_run=dry_run,
            operator=operator,
        ),
    )
    ...
    if result.data is not None:
        ...
        if result.data.artifact_path:
            review = review_shadow_strategy_experiment_registry_artifact(result.data.artifact_path)
            if not review.valid:
                payload["ok"] = False
                payload["errors"].append("SHADOW_EXPERIMENT_REGISTRY_REVIEW_FAILED")
        payload["data"] = {
            ...
            "calibration_artifact_path": result.data.calibration_artifact_path,
...
def print_shadow_experiment_registry_result(payload: dict[str, Any], *, compact: bool = False) -> None:
    data = payload.get("data") if isinstance(payload.get("data"), dict) else {}
    summary = data.get("summary") if isinstance(data.get("summary"), dict) else {}
    safety = data.get("safety") if isinstance(data.get("safety"), dict) else {}
    print(f"shadow_experiment_registry_status={payload.get('status')}")
    print("registry_contract=shadow_strategy_experiment_registry_v1")
    ...
    print(f"calibration_artifact_path={data.get('calibration_artifact_path') or 'missing'}")
    print(f"experiment_count={summary.get('experiment_count', 0)}")
    print(f"candidate_count={summary.get('candidate_count', 0)}")
    print(f"blocked_by_replay_evidence_count={summary.get('blocked_by_replay_evidence_count', 0)}")
    ...
    if not compact and data.get("artifact"):
        print("shadow_experiment_registry_json=" + json.dumps(data["artifact"], ensure_ascii=False, sort_keys=True))
